...
const register = async({
    ...
    const newUser = await User.create({
        IDU : function generateRandomId() {
            const timestamp = Date.now().toString(36); // Sử dụng timestamp và chuyển đổi sang hệ cơ số 36
            const randomString = Math.random().toString(36).substr(2); // Chuỗi ngẫu nhiên
            const randomId = timestamp + randomString; // Kết hợp timestamp và chuỗi ngẫu nhiên
            return randomId;
          },
        ho_ten: name,
        ntns: dob,
