...
const register = async({
    ...
    const newUser = await User.create({
        IDU :function generateUniqueId() {
            const timestamp =toInt( Date.now());
            const randomInt = Math.floor(Math.random() * 1000000); // Số ngẫu nhiên từ 0 đến 999999
            const uniqueId = timestamp * 100000 + randomInt; // Kết hợp timestamp và số ngẫu nhiên thành một số duy nhất
            return uniqueId;
        },
        ho_ten: name,
        ntns: dob,
